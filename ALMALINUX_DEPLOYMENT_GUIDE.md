# AlmaLinux 9.6 Deployment Guide - BigWater Affiliate System

This guide provides complete instructions for deploying the BigWater affiliate system to AlmaLinux 9.6.
...
```sql
-- Execute these commands in MySQL prompt
CREATE DATABASE bigwater_affiliate;
CREATE USER 'bigwater'@'localhost' IDENTIFIED BY 'YourStrongPassword123!';
GRANT ALL PRIVILEGES ON bigwater_affiliate.* TO 'bigwater'@'localhost';
FLUSH PRIVILEGES;
EXIT;
```
...
mysql -u bigwater -p bigwater_affiliate < AFFILIATE_DATABASE_SCHEMA.sql
```
...
quarkus.datasource.jdbc.url=jdbc:mysql://localhost:3306/bigwater_affiliate
...
mysqldump -u bigwater -p'YourStrongPassword123!' bigwater_affiliate > "$BACKUP_DIR/bigwater_affiliate_$DATE.sql"
...
2. **Database connection issues**
   ...
   # Test connection
   mysql -u bigwater -p bigwater_affiliate -e "SELECT 1;"
   ```
...
After completing these steps, your BigWater affiliate system will be fully deployed on AlmaLinux 9.6 with:

- ✅ Quarkus backend API running on port 8080
- ✅ Vue.js frontend served by Nginx
- ✅ MySQL database with affiliate schema
