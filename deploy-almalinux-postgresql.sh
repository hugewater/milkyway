...
# BigWater Affiliate System - AlmaLinux 9.6 Deployment Script (PostgreSQL)
# This script automates the deployment process using PostgreSQL
...
get_user_input() {
    ...
    read -s -p "Enter PostgreSQL password for bigwater user: " DB_PASSWORD
    echo
...
setup_database() {
    ...
    # Set PostgreSQL password for postgres user first
...
# BigWater Optimization Settings
...
EOF
    ...
    print_status "PostgreSQL optimized successfully."
}
...
display_completion() {
    ...
    echo -e "${GREEN}Your BigWater Affiliate System has been deployed successfully with PostgreSQL!${NC}"
    echo
    echo -e "${BLUE}Next Steps:${NC}"
    echo "1. Copy your application files to: $APP_DIR"
    echo "2. Apply database schema: psql -U $DB_USER -d $DB_NAME -h localhost < AFFILIATE_DATABASE_SCHEMA_POSTGRESQL.sql"
    echo "3. Update application.properties with database credentials"
    echo "4. Add PostgreSQL driver to pom.xml: quarkus-jdbc-postgresql"
    echo "5. Build and deploy backend: cd $APP_DIR/quarkus-api && ./mvnw clean package"
    echo "6. Build and deploy frontend: cd $APP_DIR/vue3 && pnpm install && pnpm build"
    echo "7. Copy frontend build to nginx: sudo cp -r dist/* /var/www/html/"
    echo "8. Start the application: sudo systemctl start bigwater-api"
    echo
    ...
    echo "Connect to database: psql -U $DB_USER -d $DB_NAME -h localhost"
    echo "Backup database: $APP_DIR/backup-db.sh"
}

# Main deployment function
main() {
    print_header "BigWater Affiliate System - AlmaLinux 9.6 Deployment (PostgreSQL)"
